...
namespace Ride
{
    public class ride
    {
        ...
        private (int, int) min(List<int> l, List<bool> l2)
        {
        ...
        public bool assignDriver(List<driver> l, vehicle v)
        {
            ...
            while (true && co > 0)
            {
                (int m, int min_idx) = min(l2, l3);
                if (l[min_idx].Availability == true && l[min_idx].Var_vehicle.Type == v.Type.ToUpper())
                {
                    varDriver = l[min_idx];
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.WriteLine("----------------------------------------------------------------------------------------------");
                    Console.WriteLine("your driver will be " + l[min_idx].Name + " with vehicle's License no: " + l[min_idx].Var_vehicle.LicensePlate);
                    return true;
                }
                else
                {
        ...
        public void giveRating(int i)
        {
            VarDriver.Rating.Add(i);
        }
